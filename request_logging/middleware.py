...
class LoggingMiddleware(object):
    ...
    def _log_request(self, request):
        method_path = "{} {} - user-{}".format(request.method, request.get_full_path(), request.user.id)
    ...
    def process_response(self, request, response):
        resp_log = "{} {} - user-{} - {}".format(request.method, request.get_full_path(), request.user.id, response.status_code)
        skip_logging_because = self._should_log_route(request)
        if skip_logging_because:
